...
// =============================================
Office.onReady(() => {
  console.log("ZipMail commands.js loaded");
});
...
// Update split-button icon according to ZIP state (none, zip, encrypted)
async function updateRibbonIcon() {
  ...
  try {
    ...
    console.error("ZipMail: Failed to update ribbon icon:", error);
  }
}

// Set ZIP mode and update icon
...
function disableZip(event) {
  setZipMode("none");
  showNotification("ZIP disabled");
  event.completed({ allowEvent: true });
}

// Enable normal ZIP (no password)
function enableZip(event) {
  setZipMode("zip");
  showNotification("ZIP enabled");
  event.completed({ allowEvent: true });
}

// Enable encrypted ZIP (prompt for password)
async function enableZipEncrypted(event) {
  const result = await getPasswordFromDialog();
  if (!result?.password) {
    showNotification("Encrypted ZIP not enabled. Current mode: " + getZipMode());
    event.completed({ allowEvent: true });
    return;
  }

  // Save password securely in memory (never localStorage)
  localStorage.setItem(ZIP_PASSWORD_KEY, result.password); // Could be replaced by a secure storage implementation

  setZipMode("encrypted");
  showNotification("Encrypted ZIP enabled");
  event.completed({ allowEvent: true });
}
...
async function onMessageSend(event) {
  ...
  try {
    ...
    if (isEncrypted) {
      const result = await getPasswordFromDialog();
      if (!result?.password) {
        showNotification("Password required — sending canceled");
        await zipWriter.close();
        event.completed({ allowEvent: false });
        return;
      }

      // Use password in memory only
      options = { ...options, password: result.password, encryptionStrength: 3 };

      // Immediately clear the password variable after use
    ...
    try {
      ...
      console.error("ZipMail: Failed to read attachments:", e);
      showNotification("Critical error: attachments inaccessible. Send blocked.");
      await zipWriter.close();
      event.completed({ allowEvent: false });
      return;
    }
    ...
    for (const att of attachments) {
      try {
        ...
        console.error("ZipMail: Failed to remove attachment:", att.name, e);
        showNotification("Critical error: cannot remove an attachment. Send blocked.");
        event.completed({ allowEvent: false });
        return;
      }
    }
    ...
    try {
      ...
      console.error("ZipMail: Failed to add msg.zip:", e);
      showNotification("Error: cannot add ZIP. Send blocked.");
      event.completed({ allowEvent: false });
      return;
    }
    ...
    try {
      ...
      console.error("ZipMail: Failed to inject message template:", err);
      showNotification("Error template: " + err.message);
      event.completed({ allowEvent: false, errorMessage: err.message });
    }
  } catch (err) {
    console.error("ZipMail: " + err.message, err);
    showNotification("ZipMail error: " + err.message);
    event.completed({ allowEvent: false, errorMessage: err.message });
  }
}
...
async function onMessageRead(event) {
  ...
  try {
    bodyHtml = await new Promise((resolve, reject) => {
      item.body.getAsync("html", (res) => {
        res.status === Office.AsyncResultStatus.Succeeded ? resolve(res.value) : reject(new Error("Failed to read HTML body"));
      });
    });
  } catch (e) {
    console.log("ZipMail: Not HTML or read error, ignored.", e);
    event.completed?.();
    return;
  }
  ...
  if (!meta) {
    console.log("ZipMail: No <meta name='zipmail'> found, ignored.");
    event.completed?.();
    return;
  }
  ...
  try {
    ...
    console.warn("ZipMail: getAttachmentsAsync failed, ignored.", e);
    event.completed?.();
    return;
  }
  ...
  if (!msgZip) {
    showNotification("ZipMail error: msg.zip missing despite meta tag.");
    console.error("ZipMail: Meta tag present but no msg.zip");
    event.completed?.();
    return;
  }
  ...
  try {
    ...
    showNotification("Error: cannot read msg.zip");
    console.error("ZipMail: Failed to read msg.zip", e);
    event.completed?.();
    return;
  }
  ...
  try {
    ...
    const result = await getPasswordFromDialog(); // no default
    if (!result?.password) {
      showNotification("Password required to unzip.");
      await reader.close();
      event.completed?.();
      return;
    }
    ...
    try {
      ...
      console.error("ZipMail: Decryption failed", err);
      await reader.close();
      event.completed?.();
      return;
    } finally {
      result.password = null; // clear immediately
    }
  }

  const messageEntry = entries.find((e) => e.filename.toLowerCase() === "message.htm");
  if (!messageEntry) {
    showNotification("Error: message.htm missing in msg.zip");
    console.error("ZipMail: message.htm not found");
    await reader.close();
    event.completed?.();
    return;
  }
  ...
  try {
    ...
    showNotification("Error: cannot read message.htm");
    console.error("ZipMail: Failed to read message.htm", e);
    await reader.close();
    event.completed?.();
    return;
  }
  ...
  try {
    await new Promise((resolve) => item.body.setAsync(originalHtml, { coercionType: "html" }, resolve));
  } catch (e) {
    showNotification("Error: cannot restore body");
    console.error("ZipMail: Failed to setAsync body", e);
    await reader.close();
    event.completed?.();
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.warn(`ZipMail: Failed to add attachment ${entry.filename}`, e);
    }
  }
  ...
  try {
    ...
    console.warn("ZipMail: Failed to remove msg.zip", e);
  }

  await reader.close();

  setZipMode(meta.encrypted ? "encrypted" : "zip");
  showNotification("ZipMail message restored automatically.");
  event.completed?.();
}
